...
# include "sort.h"
using namespace std;
// Изменить кодировку, при открытии не показывается русские символы
// find_last_of функция проверить
// transform(filename.begin(), filename.end(), filename.begin(), [](unsigned char c) {return static_cast<unsigned char>(toupper(c)); }) - объяснить подробно
// Отдельный метод для вывода массива и его сортировки, и сохранения в файл
...
int Get_Length_Array() { //переименовать
	int length = 0;
